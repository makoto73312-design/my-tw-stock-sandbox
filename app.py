...
for t in raw_list:
    if t.isdigit():
        processed_tickers.append(f"{t}.TW")
    else:
...
def run_backtest_engine(df, strategy_name, days, posture):
    ...
    if "🚀 大膽進攻型" in posture:
        ...
        rsi_max, vol_mult, dip_pct, rsi_min, chip_col = 65, 1.40, -0.12, 25, '主力籌碼_Q95'
    else: 
    ...
    if total_return > 0 and total_trades > 0:
        if total_return >= 0.20 and final_win_rate >= 0.52: stars = "⭐⭐⭐⭐⭐ (首選)"
...
with tab_summary:
    if st.button("🚀 啟動全自動台股核心回測引擎", use_container_width=True):
        with st.spinner(f"正在抓取台灣市場數據並執行視覺優化排列..."):
...
with tab_debug:
    ...
    if st.session_state.calculated:
        ...
        with col_tk: debug_ticker = st.selectbox("🎯 選擇想檢查的台股代號", raw_list)
        ...
        if db_key in st.session_state.detail_db:
            ...
            if len(buys) > 0:
                fig.add_trace(go.Scatter(x=[b[0] for b in buys], y=[b[1] for b in buys], mode='markers', name='🟢 BUY (進場)', marker=dict(symbol='triangle-up', size=12, color='#00FF00'), hovertemplate='<b>進場買點</b><br>日期: %{x|%Y-%m-%d}<br>價格: NT$%{y:.2f}<br>當日RSI: %{customdata[0]:.1f}<extra></extra>', customdata=[[float(v_df.loc[b[0], 'RSI_14'])] for b in buys]))
            if len(sells) > 0:
                fig.add_trace(go.Scatter(x=[s[0] for s in sells], y=[s[1] for s in sells], mode='markers', name='🔴 SELL (出場)', marker=dict(symbol='triangle-down', size=12, color='#FF0000'), hovertemplate='<b>風控出場點</b><br>日期: %{x|%Y-%m-%d}<br>價格: NT$%{y:.2f}<extra></extra>'))
            fig.update_layout(title=f"<b>台股 {debug_ticker} - {debug_strat} 軌跡圖</b>", xaxis_title="交易日期", yaxis_title="股價 (NT$)", hovermode="x unified", template="plotly_dark")
